...
"font: 75 12pt \"MS Shell Dlg 2\";")
		...
		self.list1.itemDoubleClicked.connect(self.removelist1) #remove and add
...
"color: rgb(255, 255, 255);")
		...
		self.list2.itemDoubleClicked.connect(self.removelist2)        #add and remove l2
		...
		self.menuFile.addAction(self.actionNew) #connecting with function
	...
	def menufunction(self, action):
		...
		if txt=="New Team":        	## Reset the class attributes
			...
			self.list1.clear()
			self.list2.clear()
			self.l1.setText("???") #list 1 in other module
			self.showstatus()
...
## Reset the class attributes
	...
	def openteam(self):
		...
		for row in cur:
			teams.append(row[0])
		team, ok = QtWidgets.QInputDialog.getItem(MainWindow, "Dream Team Selector","Choose a team", teams, 0, False)
		if ok and team:
			self.l1.setText(team) #other module
	...
	def fillList(self,ctgr):
		...
		for row in cursor:
			...
			for i in range(self.list2.count()):
				selected.append(self.list2.item(i).text()) #count
	...
	def removelist2(self, item):
		...
		"""
		self.list2.takeItem(self.list2.row(item)) #to get the points
